...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) >0): # enforce the user to provide both names
        ...
        with conn:
            cursor = conn.cursor()
            # Check the database for existance of the fullname, if so we will alert user and disreguard request
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_phonebook WHERE col_fullname = '{}'""".format(var_fullname))#,(var_fullname))
            count = cursor.fetchone()[0]
            chkName = count
            if chkName == 0: #if this is 0 then there is no existence if the fullname and we can add new data
...
def onDelete(self):
    ...
    with conn:
        ...
        count = cur.fetchone()[0]
        if count > 1:
            confirm = messagebox.askokcancel("Delete Confirmation", "All information associated with, ({}) \nwill be permenantly deleted for the database. \n\nProceed with the deletion request?".format(var_select))
            if confirm:
                conn = sqlite3.connect('phonebook.db')
                with conn:
                    cursor = conn.cursor()
...
#####               on refresh(self) # update the listbox of the changes
                conn.commit()
        else:
...
def onUpdate(self):
    ...
    var_phone = self.txt_phone.get().strip() # normalize the data to maintain data integrity
    var_email = self.txt_email.get().strip()
